...
public class blockController1 : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (gameObject.GetComponent<Rigidbody2D>().isKinematic == false)
        {
            ...
            {
                this.delta = 0;
                //transform.position += new Vector3(0, -0.255f, 0);
                //transform.position += new Vector3(0, -0.255f, 0);
            ...
            {
                //transform.position += new Vector3(-0.51f, 0, 0);
                transform.position += new Vector3(-0.250f, 0, 0);
                transform.position += new Vector3(-0.250f, 0, 0);
            ...
            else if(Input.GetMouseButtonUp(0) && blockGenerator.state == 0)
            {
                ...
                //else if ((endPos.x >= 260f && endPos.x <= 2280f) ||
            ...
            if (rotation==0 && judge)
            {
                //transform.RotateAround(transform.position + new Vector3(-0.3f, 0, 0), Vector3.forward, 90);
                transform.RotateAround(transform.position + new Vector3(0, 0, 0), Vector3.forward, 90);
                rotation = 1;
                 this.aud.PlayOneShot(this.rotateSE);
                 judge = false;
            }
            else if (rotation == 1 && judge)
            {
                 //transform.RotateAround(transform.position + new Vector3(0, -0.3f, 0), Vector3.forward, -90);
                 transform.RotateAround(transform.position + new Vector3(0, 0, 0), Vector3.forward, -90);
                 rotation = 0;
                 this.aud.PlayOneShot(this.rotateSE);
                 judge = false;
